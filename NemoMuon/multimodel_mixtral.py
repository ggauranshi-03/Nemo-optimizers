...
#     """
#     Streaming multimodal dataset from Hugging Face.
#     Uses COCO Captions dataset as an example.
#     """
...
#         print(f"[DATASET] Loading multimodal dataset (split={split})...")
        
#         # Using COCO Captions dataset - smaller and easier to stream
#         # Alternative: "nlphuji/flickr30k" or "ydshieh/coco_dataset_script"
#         try:
...
#         except Exception as e:
#             print(f"[DATASET] Error loading COCO: {e}")
#             print("[DATASET] Falling back to conceptual captions...")
#             # Fallback to Conceptual Captions
#             self.dataset = load_dataset(
#                 "conceptual_captions",
#                 split=split,
#                 streaming=True
...
#         print(f"[DATASET] Loaded {len(self.dataset_list)} samples")
...
#         if 'caption' in item:
#             text = item['caption']
#         elif 'sentences' in item and 'raw' in item['sentences']:
#             text = item['sentences']['raw'][0] if isinstance(item['sentences']['raw'], list) else item['sentences']['raw']
#         else:
#             text = "A photo"
...
#         # Create attention_mask (all ones for non-padded tokens)
...
#     """
#     Streaming multimodal dataset from Hugging Face.
#     Uses COCO Captions dataset with fallback options.
#     """
#     def __init__(self, tokenizer, seq_length=1024, split='train', max_samples=10000):
#         self.tokenizer = tokenizer
#         self.seq_length = seq_length
...
#         # Try multiple COCO dataset options in order of preference
#         dataset_loaded = False
        
#         # Option 1: Try sentence-transformers/coco-captions (no script, works well)
#         if not dataset_loaded:
#             try:
#                 print("[DATASET] Attempting to load sentence-transformers/coco-captions...")
#                 self.dataset = load_dataset(
#                     "sentence-transformers/coco-captions",
#                     split=split if split == "train" else "train",  # Only has train split
#                     streaming=True,
...
#                 self.dataset_list = list(self.dataset)
#                 dataset_loaded = True
#                 print(f"[DATASET] Successfully loaded sentence-transformers/coco-captions")
#             except Exception as e:
#                 print(f"[DATASET] Failed to load sentence-transformers/coco-captions: {e}")
        
#         # Option 2: Try yerevann/coco-karpathy (alternative COCO)
#         if not dataset_loaded:
#             try:
#                 print("[DATASET] Attempting to load yerevann/coco-karpathy...")
#                 self.dataset = load_dataset(
...
#                 self.dataset_list = list(self.dataset)
#                 dataset_loaded = True
#                 print(f"[DATASET] Successfully loaded yerevann/coco-karpathy")
#             except Exception as e:
#                 print(f"[DATASET] Failed to load yerevann/coco-karpathy: {e}")
        
#         # Option 3: Fallback to conceptual captions (12M dataset)
#         if not dataset_loaded:
#             try:
#                 print("[DATASET] Falling back to conceptual_captions...")
#                 self.dataset = load_dataset(
#                     "conceptual_captions",
...
#                 self.dataset_list = list(self.dataset)
#                 dataset_loaded = True
#                 print(f"[DATASET] Successfully loaded conceptual_captions")
#             except Exception as e:
#                 print(f"[DATASET] Failed to load conceptual_captions: {e}")
        
#         # Option 4: Last resort - WIT (Wikipedia Image-Text)
#         if not dataset_loaded:
#             try:
#                 print("[DATASET] Falling back to WIT (Wikipedia Image-Text)...")
#                 self.dataset = load_dataset(
#                     "wikimedia/wit_base",
#                     split="train",
#                     streaming=True,
...
#                 self.dataset_list = list(self.dataset)
#                 dataset_loaded = True
#                 print(f"[DATASET] Successfully loaded WIT")
#             except Exception as e:
#                 print(f"[DATASET] All dataset options failed: {e}")
#                 raise RuntimeError("Could not load any multimodal dataset")
...
#         # Extract text (caption) - handle different dataset formats
#         if 'caption' in item:
#             text = item['caption']
#         elif 'sentences' in item:
#             # Handle COCO format with sentences
#             if isinstance(item['sentences'], dict) and 'raw' in item['sentences']:
#                 raw = item['sentences']['raw']
#                 text = raw[0] if isinstance(raw, list) else raw
#             elif isinstance(item['sentences'], list):
#                 text = item['sentences'][0]
#             else:
#                 text = str(item['sentences'])
#         elif 'set' in item:  # sentence-transformers format
#             # This dataset has pairs, take first caption
#             text = item['set'][0] if isinstance(item['set'], list) and len(item['set']) > 0 else "A photo"
#         elif 'text' in item:
#             text = item['text']
#         else:
#             text = "A photo"
...
#                 # For datasets with image URLs - create dummy for now
#                 # In production, you'd download the image
#                 image = Image.new('RGB', (224, 224), color='white')
#             else:
#                 # Create dummy image if not available
#                 image = Image.new('RGB', (224, 224), color='white')
