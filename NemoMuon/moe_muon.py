...
class Muon(Optimizer):
    """
    Hybrid optimizer: Muon for linear weights, AdamW for everything else.
    ...
    AdamW uses standard adaptive optimization for biases, norms, and embeddings.
    """
    def __init__(
        self,
        params,
    ...
    def _classify_param(self, p):
        """Determine if parameter should use Muon (True) or AdamW (False)"""
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if use_muon:
                    ...
                    # Run Newton-Schulz orthogonalization
                    g_ortho = zeropower_via_newtonschulz5(g, steps=ns_steps)
                    
                    # Scale by aspect ratio (max(1, rows/cols)^0.5)
                    rows, cols = g.size()
...
# ============================================================================ #
def main():
    parser = argparse.ArgumentParser(description="NeMo MoE Pretraining with Muon+AdamW")
    parser.add_argument("--name", type=str, default="moe_muon", help="Experiment name")
    ...
    print(f"\n{'='*70}")
    print(f"[START] MoE Training with Muon+AdamW")
    print(f"{'='*70}\n")
